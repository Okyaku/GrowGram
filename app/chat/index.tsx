...
export default function ChatInboxScreen() {
  ...
  const loadInbox = React.useCallback(async () => {
    try {
      ...
      const profileItems =
        (profileResponse as { data?: { listProfiles?: { items?: Array<CloudProfile | null> } } }).data
          ?.listProfiles?.items ?? [];
      const receiptItems =
        (receiptResponse as { data?: { listReadReceipts?: { items?: Array<CloudReceipt | null> } } }).data
          ?.listReadReceipts?.items ?? [];

      const profileByOwner = new Map<string, { id: string; username: string; avatarUrl?: string }>();
        ...
        profileItems
          ...
          .map(async (item) => {
            ...
            return {
              ...
              id: item.id,
              username: item.username ?? (item.owner ?? item.id).split("@")[0].toUpperCase(),
              avatarUrl,
            };
          }),
      );
      ...
      const readMessageIds = new Set(
        receiptItems
          .filter((item): item is CloudReceipt => Boolean(item?.id && item.messageId && item.readerId))
      ...
      normalizedMessages.forEach((message) => {
        const partnerId = message.fromUserId === me ? message.toUserId : message.fromUserId;
        ...
        const shouldReplace = !current || new Date(lastAt).getTime() > new Date(current.lastAt).getTime();
        const unreadCount =
        ...
        if (!current || shouldReplace) {
          conversationMap.set(partnerId, {
            partnerId,
            partnerName: profile?.username || partnerId.split("@")[0].toUpperCase(),
    ...
    <ScreenContainer backgroundColor={theme.colors.surface} scrollable={false}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <Pressable style={styles.row} onPress={() => router.push(`/chat/${item.partnerId}`)}>
        ...
        ListEmptyComponent={
          isLoading ? null : (
            <View style={styles.emptyCard}>
              <Ionicons name="chatbubble-ellipses-outline" size={24} color={theme.colors.textSub} />
              <Text style={styles.emptyTitle}>まだメッセージがありません</Text>
              <Text style={styles.emptySub}>プロフィールやストーリーからDMを送ってみてください。</Text>
            </View>
          )
        }
      />
