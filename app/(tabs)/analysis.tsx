...
const chartData: Record<
  Period,
  { points: string; delta: string; xLabels: string[]; path: string; ranks: { icon: React.ComponentProps<typeof Ionicons>['name']; title: string; value: string; sub: string }[] }
> = {
  day: {
    points: '126 pts',
    delta: '+4.8%',
  ...
  year: {
    ...
    ranks: [
      ...
      { icon: 'checkmark-circle', title: '継続', value: '2位', sub: '/ 2,200人' },
    ],
  },
};

const leaders = [
  { rank: 1, name: 'Kenji Tanaka', id: '@KNTNKA', points: 4820, avatar: 'https://images.unsplash.com/photo-1544723795-3fb6469f5b39?w=200' },
  { rank: 2, name: 'Aria Sato', id: '@ARIASATO', points: 4150, avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=200' },
  { rank: 3, name: 'Kaito Ito', id: '@KTOITO', points: 3980, avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=200' },
];
  ...
  return (
    <ScreenContainer backgroundColor={theme.colors.background}>
      <View style={styles.headerRow}>
        ...
        <Pressable style={styles.guideBtn} onPress={() => router.push('/analysis-guide')}>
      ...
      <View style={styles.segment}>
        {(['day', 'month', 'year'] as const).map((item) => (
          <Pressable key={item} style={[styles.segmentItem, period === item && styles.segmentActive]} onPress={() => setPeriod(item)}>
            <Text style={[styles.segmentText, period === item && styles.segmentTextActive]}>{periodLabels[item]}</Text>
          </Pressable>
        ))}
      </View>
      ...
      <View style={styles.chartCard}>
        ...
        <View style={styles.graphWrap}>
          <Svg width='100%' height={120} viewBox='0 0 320 120'>
            <Path d={data.path} stroke={theme.colors.primary} strokeWidth={3.5} fill='none' strokeLinecap='round' />
          </Svg>
        ...
        {data.ranks.map((rank) => (
          <View key={rank.title} style={styles.rankCard}>
            <View style={styles.rankIconWrap}>
              <Ionicons name={rank.icon} size={14} color={theme.colors.primary} />
            </View>
      ...
      {leaders.map((leader) => (
        <View key={leader.rank} style={styles.leaderRow}>
          <View style={[styles.rankCircle, leader.rank === 1 && styles.rankCircle1, leader.rank === 2 && styles.rankCircle2, leader.rank === 3 && styles.rankCircle3]}>
          ...
          <Text style={styles.leaderPoints}>{leader.points.toLocaleString()}</Text>
        </View>
      ))}
    </ScreenContainer>
  );
