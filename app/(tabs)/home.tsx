...
export default function HomeScreen() {
  ...
  const { canCreatePost, postCredits, streakDays, level, totalScore, recordDailyActivity, adjustScore } = useRoadmap();
  ...
  const [reactionKeys, setReactionKeys] = React.useState<Set<string>>(new Set());
  const [savedPostIds, setSavedPostIds] = React.useState<Set<string>>(new Set());
  const [reactionRecordIdByKey, setReactionRecordIdByKey] = React.useState<Record<string, string>>({});
  const [saveRecordIdByPost, setSaveRecordIdByPost] = React.useState<Record<string, string>>({});
  ...
  const tapCountRef = React.useRef<Record<string, { count: number; timer?: ReturnType<typeof setTimeout> }>>({});
  ...
  const loadFeed = React.useCallback(async () => {
    try {
      ...
      const [postsResponse, profilesResponse, likesResponse, savesResponse, storiesResponse] = await Promise.all([
      ...
      const profileItems =
        (profilesResponse as { data?: { listProfiles?: { items?: Array<CloudProfile | null> } } }).data?.listProfiles
          ?.items ?? [];
      const likeItems =
        (likesResponse as { data?: { listPostLikes?: { items?: Array<CloudLike | null> } } }).data?.listPostLikes?.items ?? [];
      const saveItems =
        (savesResponse as { data?: { listPostSaves?: { items?: Array<CloudSave | null> } } }).data?.listPostSaves?.items ?? [];
      const storyItems =
        (storiesResponse as { data?: { listStories?: { items?: Array<CloudStory | null> } } }).data?.listStories?.items ?? [];
      ...
      const profileByOwner = new Map(profileWithAvatar.map((profile) => [profile.owner, profile]));
      ...
      likeItems
        ...
        .forEach((item) => {
          ...
          if (reactionType === 'passion') {
            passionCountByPost[item.postId] = (passionCountByPost[item.postId] ?? 0) + 1;
          }
          if (reactionType === 'logic') {
            logicCountByPost[item.postId] = (logicCountByPost[item.postId] ?? 0) + 1;
          }
          if (reactionType === 'routine') {
            routineCountByPost[item.postId] = (routineCountByPost[item.postId] ?? 0) + 1;
          }
      ...
      saveItems
        ...
        .forEach((item) => {
          saveCountByPost[item.postId] = (saveCountByPost[item.postId] ?? 0) + 1;
          if (owner && item.owner === owner) {
      ...
      const normalizedPosts = postItems
        ...
        .map((item) => {
          ...
          return {
            id: item.id,
            userId: postOwner,
            userName: profile?.username || postOwner.split('@')[0].toUpperCase(),
      ...
      const storyList = await Promise.all(
        Array.from(storyByOwner.values()).map(async (story) => {
          let imageUrl = 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=300';
          try {
          ...
          return {
            id: story.id,
            owner: story.owner ?? '',
            userName: profile?.username || (story.owner ?? 'USER').split('@')[0].toUpperCase(),
            image: imageUrl,
      ...
      const myStoryItem: StoryItem = hasMyStory
        ...
        : {
            ...
            image: myProfile?.avatarUrl || 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=300',
            active: true,
          };
  ...
  const toggleReaction = React.useCallback(async (postId: string, reactionType: ReactionType) => {
    try {
      const key = `${postId}:${reactionType}`;
      const existingId = reactionRecordIdByKey[key];
      if (existingId) {
        await client.graphql({ query: deletePostLikeMutation, variables: { input: { id: existingId } } });
        adjustScore(-30);
      } else {
        await client.graphql({ query: createPostLikeMutation, variables: { input: { postId, reactionType } } });
  ...
  const showGestureFeedback = React.useCallback((postId: string, type: ReactionType) => {
  ...
  const onPostLongPress = React.useCallback((postId: string) => {
  ...
  const onPostTap = React.useCallback((postId: string) => {
  ...
  const toggleSave = React.useCallback(async (postId: string) => {
    try {
      ...
      if (existingId) {
        await client.graphql({ query: deletePostSaveMutation, variables: { input: { id: existingId } } });
      } else {
        await client.graphql({ query: createPostSaveMutation, variables: { input: { postId } } });
      }
      await loadFeed();
    } catch (error) {
  ...
  const onArchive = React.useCallback((post: FeedPost) => {
    Alert.alert('アーカイブ', 'この投稿をアーカイブしますか？', [
      { text: 'キャンセル', style: 'cancel' },
      {
        ...
        onPress: () => {
          void (async () => {
            try {
              await client.graphql({ query: updatePostMutation, variables: { input: { id: post.id, isArchived: true } } });
              await loadFeed();
            } catch (error) {
  ...
  const onDelete = React.useCallback((post: FeedPost) => {
    Alert.alert('投稿削除', 'この投稿を削除しますか？', [
      { text: 'キャンセル', style: 'cancel' },
      {
        ...
        onPress: () => {
          void (async () => {
            try {
              await client.graphql({ query: deletePostMutation, variables: { input: { id: post.id } } });
              await loadFeed();
            } catch (error) {
  ...
  const onRepost = React.useCallback(async (post: FeedPost) => {
    try {
  ...
  return (
    <ScreenContainer backgroundColor={theme.colors.surface}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.storyList}>
        {stories.map((story) => (
          <Pressable
            ...
            onPress={() => router.push(story.id === 'my-create' ? '/story-create' : `/story/${story.id}`)}
          >
      ...
      <View style={styles.actionRow}>
        <CustomButton label='ストーリー投稿' onPress={() => router.push('/story-create')} style={styles.actionButton} />
        <CustomButton
        ...
        return (
          <View key={post.id} style={styles.card}>
            <View style={styles.cardHeader}>
              <Pressable style={styles.userRow} onPress={() => router.push(`/profile/${post.userId}`)}>
                {post.userAvatar ? <Image source={{ uri: post.userAvatar }} style={styles.userAvatar} /> : <View style={styles.userAvatarPlaceholder} />}
                <Text style={styles.user}>{post.userName}</Text>
              </Pressable>
            </View>

            <Text style={styles.day}>{post.title}</Text>
            <Pressable onLongPress={() => onPostLongPress(post.id)} onPress={() => onPostTap(post.id)} style={styles.imagePressable}>
              ...
              {gestureFeedback?.postId === post.id ? (
                ...
                >
                  <Ionicons name={gestureFeedback.icon} size={18} color={theme.colors.onPrimary} />
                  <Text style={styles.gesturePopupText}>{gestureFeedback.label}</Text>
                </View>
              ) : null}
            </Pressable>
            <Text style={styles.gestureHint}>長押し: 情熱 / 2タップ: 論理 / 3タップ: 一貫性</Text>
            ...
            <View style={styles.scoreRow}>
              ...
              <View style={styles.scoreItem}>
                ...
                <Text style={styles.scoreLabel}>一貫性 {post.routineCount}</Text>
              </View>
            </View>

            <View style={styles.postActions}>
              <Pressable style={styles.postActionItem} onPress={() => void toggleSave(post.id)}>
                <Ionicons name={savedPostIds.has(post.id) ? 'bookmark' : 'bookmark-outline'} size={16} color={theme.colors.textSub} />
                <Text style={styles.postActionText}>保存 {post.saveCount}</Text>
              </Pressable>
              <Pressable style={styles.postActionItem} onPress={() => void onRepost(post)}>
                <Ionicons name='repeat-outline' size={16} color={theme.colors.textSub} />
                <Text style={styles.postActionText}>再投稿</Text>
              </Pressable>
              {isOwner ? (
                <Pressable style={styles.postActionItem} onPress={() => onArchive(post)}>
                  <Ionicons name='archive-outline' size={16} color={theme.colors.textSub} />
                  <Text style={styles.postActionText}>アーカイブ</Text>
                </Pressable>
              ) : null}
              {isOwner ? (
                <Pressable style={styles.postActionItem} onPress={() => onDelete(post)}>
                  <Ionicons name='trash-outline' size={16} color={theme.colors.danger} />
                  <Text style={[styles.postActionText, { color: theme.colors.danger }]}>削除</Text>
                </Pressable>
              ) : null}
            </View>
          </View>
