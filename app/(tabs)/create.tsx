        ...
        <View style={styles.rowBetween}>
          <View style={styles.objectiveTextWrap}>
            ...
            <Text style={styles.objectiveDate}>
              {activeRoadmap.mode === "ai-auto"
                ? "AI自動生成ロードマップ"
                : `レベル: ${activeRoadmap.level}`}
            </Text>
          </View>
      ...
      {activePostCredits > 0 ? (
        <View style={styles.noticeCard}>
          ...
          <Text style={styles.noticeText} numberOfLines={1}>
            投稿できます:{" "}
            {activeUnlockedMilestones.map((item) => item.title).join(" / ")}
          </Text>
        </View>
      ) : null}
      ...
      <View style={styles.roadmapWrap}>
        ...
        {renderMilestones.map((milestone, index) => {
          ...
          return (
            <View
              key={milestone.id}
              style={[styles.nodeGroup, { top: pos.top, left: pos.left }]}
            >
              <View
                style={[
                  styles.nodeIcon,
                  milestone.status === "completed" && styles.nodeCompleted,
                  milestone.status === "current" && styles.nodeCurrent,
                ]}
              >
                <Ionicons
                  name={
              ...
              >
                <Text
                  style={[
                    styles.nodeStatus,
                    milestone.status === "current" && styles.currentText,
                    milestone.status === "completed" && styles.completeText,
                  ]}
                >
                  {milestone.status === "current"
                    ? "CURRENT"
                ...
                {canTap ? (
                  <Text style={styles.tapHint}>タップして投稿</Text>
                ) : null}
              </Pressable>
            </View>
          );
