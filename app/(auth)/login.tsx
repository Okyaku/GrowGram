...
const isUnauthenticatedError = (error: unknown) => {
  ...
  return (
    name === "UserUnAuthenticatedException" || name === "NotAuthorizedException"
  );
};
...
export default function LoginScreen() {
  ...
  React.useEffect(() => {
    ...
    const loadCurrentUser = async () => {
      try {
        ...
        if (__DEV__) {
          console.log(
            "[Auth][Login] failed to fetch current user:",
            getAuthErrorDebugText(error),
          );
        }
      }
    };
  ...
  const onConfirmCode = React.useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        "確認完了",
        "ログイン処理が未完了です。再度ログインをお試しください。",
      );
    } catch (error) {
      if (__DEV__) {
        console.log(
          "[Auth][Login] confirmSignUp failed:",
          getAuthErrorDebugText(error),
        );
      }
      const message = getAuthErrorMessage(error);
  ...
  const onResendCode = React.useCallback(async () => {
    ...
    try {
      ...
      if (__DEV__) {
        console.log(
          "[Auth][Login] resendSignUpCode failed:",
          getAuthErrorDebugText(error),
        );
      }
      const message = getAuthErrorMessage(error);
    ...
    <ScreenContainer>
      <View style={styles.brandRow}>
        <View style={styles.brandIcon}>
          <Ionicons name="flower" size={18} color={theme.colors.primary} />
        </View>
      ...
      <Text style={styles.subtitle}>
        GrowGramにログインして、積み上げを続けましょう
      </Text>
      {userEmail ? (
        <Text style={styles.currentUserText}>
          現在のログインユーザー: {userEmail}
        </Text>
      ) : null}
